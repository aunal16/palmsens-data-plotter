...
%% find indices of the voltage values closest to VFixed

% Enough if 1) single sweep or 2) double sweep with VFixed a limit value.
% Returns only one min value and index, even if identicals exist.
xDifAbs = abs(x-VFixed);
[~, minDifIdx] = min(xDifAbs); 
% In case double sweep and VFixed is NOT a limit value
if strcmpi(sweepDir, "double") & ~isVFixedLim
    xDifAbs(minDifIdx) = Inf; % eliminate the first element closest to VFixed
    [~, minDif2Idx] = min(xDifAbs);
    minDifIdx = [minDifIdx minDif2Idx];
    clear xDifAbs minDif2Idx;
end

xPlt = 1:numPlots;
yPlt = zeros(1, numPlots);
for i = 1:numPlots
    yPlt(i) = mean(yArrays{i}(minDifIdx));
end

%% plot
yyaxis left
plot(ax, xPlt, yPlt, 'o-', DisplayName=("I at V="+num2str(VFixed)), Tag="dataline");
xlim(ax, [1, numPlots]);
